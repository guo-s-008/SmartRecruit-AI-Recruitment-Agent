...
def verify_interview_token(token):
    ...
    try:
        ...
        expired_at = datetime.fromisoformat(record['expired_at']) if record['expired_at'] else None
        if expired_at and datetime.now() > expired_at:
            cursor.execute("UPDATE interview_record SET status='expired' WHERE token=?", (token,))
            conn.commit()
...
def update_interview_status(token, status):
    ...
    try:
        conn = get_db_connection()
        cursor = conn.cursor()
        cursor.execute("UPDATE interview_record SET status=?, updated_at=? WHERE token=?",
                      (status, datetime.now().isoformat(), token))
        conn.commit()
        conn.close()
    except Exception as e:
        print(f"更新面试状态失败: {e}")
...
def update_module_questions(token, module_name, questions):
    ...
    try:
        conn = get_db_connection()
        cursor = conn.cursor()
        sql = f"UPDATE interview_record SET {field_name}=? WHERE token=?"
        cursor.execute(sql, (questions_json, token))
        conn.commit()
        conn.close()
    except Exception as e:
        print(f"更新面试题目失败: {e}")
...
def score_interview(token, answers, record):
    ...
    try:
        ...
        try:
            conn = get_db_connection()
            cursor = conn.cursor()
            cursor.execute("UPDATE interview_record SET final_score=?, result=? WHERE token=?",
